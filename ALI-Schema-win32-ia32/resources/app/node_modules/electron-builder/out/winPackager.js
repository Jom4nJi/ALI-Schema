...
class WinPackager extends platformPackager_1.PlatformPackager {
    ...
    pack(outDir, arch, targets, postAsyncTasks) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const packOptions = yield this.computePackOptions(outDir, appOutDir, arch);
            yield this.doPack(packOptions, outDir, appOutDir, arch, this.platformSpecificBuildOptions);
            yield this.sign(path.join(appOutDir, `${ this.appInfo.productName }.exe`));
    ...
    packageInDistributableFormat(outDir, appOutDir, arch, targets, promises) {
        for (let target of targets) {
            if (target instanceof squirrelWindows_1.default) {
                ...
                promises.push(log_1.task(`Building NSIS installer`, target.build(arch, outDir, appOutDir)));
            } else {
                ...
                promises.push(this.archiveApp(format, appOutDir, outFile).then(() => this.dispatchArtifactCreated(outFile, this.generateName1(format, arch, "win", true))));
            }
        }
    }
}
